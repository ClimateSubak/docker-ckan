...
class QaHideDatasetsAction(IQaAction):
    ...
    def run(cls, pkg_ids, form_vars):
        ...
        for pkg_id in pkg_ids:
            try:
                delete_package({"ignore_auth": True, "user": None}, {"id": pkg_id})
            except Exception as e:
                log.error(
                    f"Could not patch package in QaHideDatasetsAction.run: {pkg_id}, {e}"
                )
...
class QaUpdateDatasetsAction(IQaAction):
    ...
    def run(cls, pkg_ids, form_vars):
        ...
        for pkg_id in pkg_ids:
            try:
                ...
                log.error(
                    f"Could not patch package in QaUpdateDatasetsAction.run: {pkg_id}, {e}"
                )
    ...
    def filter_form_fields(cls, form_vars):
        ...
        fields = {
            field[0]: field[1] if field[0] == "subak_countries" else field[1][0]
            for field in filter(
                lambda field: field[0] != "id" and not (field[0].startswith("action.")),
                form_vars.items(),
            )
        }
        ...
        for key, value in fields.items():
            if value == "" or (key == "license_id" and value == "notspecified"):
                continue
    ...
    def run(cls, pkg_ids, form_vars):
        ...
        for pkg_id in pkg_ids:
            try:
                ...
                log.error(
                    f"Could not patch package in QaCleanDatasetsAction.run: {pkg_id}, {e}"
                )
