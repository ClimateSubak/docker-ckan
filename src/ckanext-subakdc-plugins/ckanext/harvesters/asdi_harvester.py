...
class ASDIHarvester(HarvesterBase):
    ...
    def gather_stage(self, harvest_job):
        ...
        r = requests.get(url, params={"Header": "Accept: application/vnd.github.v3+json"})
            ...
            try:
                ...
                if 'Description' in dataset.keys():
                    ...
                    if 'Documentation' in dataset.keys():
                        description += md("<br><h3>Documentation</h3><br>") + md(dataset['Documentation'])

                    # Append UpdateFrequency because it is markdown text, not our vocabulary
                    if 'UpdateFrequency' in dataset.keys():
                        description += md("<br><h3>Update Frequency</h3><br>") + md(dataset['UpdateFrequency'])

                    # Append Citation because it is markdown text, not our vocabulary
                    if 'Citation' in dataset.keys():
                        description += md("<br><h3>Citation</h3><br>") + md(dataset['Citation'])

                    # Append Citation because it is markdown text, not our vocabulary
                    if 'License' in dataset.keys():
                        description += md("<br><h3>License</h3><br>") + md(dataset['License'])
                        ...
                        if remote_orgs == 'create':
                            try:
                                org = tk.get_action('organization_create')(
                                    {'ignore_auth': True, 'user': self._get_user_name()},
                                    {'name': org_name,
                                     'title': remote_org})
